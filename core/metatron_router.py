...
class HealthPredictor:
    """Simple health predictor for internal routing"""
    def __init__(self):
        self.training_data = []
    ...
    def predict(self, cpu: float, mem: float, latency: float, io: float) -> Optional[float]:
        ...
        # Simple weighted average based on recent data
        weights = [0.35, 0.35, 0.15, 0.05, 0.1]  # cpu, mem, latency, io, success
        recent_avg = np.mean([d[4] for d in self.training_data[-10:]]) if self.training_data else 0.8
        
        health_score = (
            weights[0] * (1 - cpu / 100) +
            weights[1] * (1 - mem / 100) +
            weights[2] * (1 - min(latency / 1000, 1)) +
            weights[3] * (1 - min(io / 1000, 1)) +
            weights[4] * recent_avg
        )
        return health_score
